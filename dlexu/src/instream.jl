...
function score_proclivity(logfile::String, tstpop::Int64, match_margin::Float64, dlexuencl::Any)::Bool
    ...
    enclscores = count_encl_occurrence(samplepopulation,dlexuencl)
...
function count_encl_occurrence(samplepopulation,dlexuencl)::structs.EnclProbs
    ...
    if samplesize > 0       
       ...
       for i in eachindex(dlexuencl.enclpairs)
           enclpaircnt = enclpaircnt + div(length(findall(contains(dlexuencl.enclpairs[i].encl1),samplepopulation)) + 
                                       length(findall(contains(dlexuencl.enclpairs[i].encl2),samplepopulation)),2)
           enclscore = structs.EnclScore(dlexuencl.enclpairs[i].encl1,dlexuencl.enclpairs[i].encl2,enclpaircnt/samplesize)
